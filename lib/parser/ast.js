...
Expression.prototype.compile = function(locals, opts) {
  ...
  if (this.filter && opts && opts.filter && this.filter.name in opts.filter) {
    ref.filter = {
      ...
      args: this.filter.args && this.filter.args.map(function(arg) {
...
Reference.prototype.get = function() {
  ...
  if (this.filter) {
    ...
    var args = this.filter.args && this.filter.args.map(function(arg) {
      return arg instanceof Reference ? arg.get() : args
    }) || []
    result = fn.apply({ key: this.key, obj: this.obj }, [result].concat(args))
  }
  return result
}
